...
def procesar_jornada(jornada,competicion):
    ...
    df_raw = pd.read_csv(f'data/raw/new/jornada_{jornada}_{competicion}.csv')
    ...
    partidos_nuevo = pd.merge(partidos_nuevo,
                        ...
                        how = 'left') \
                .drop(['numero_jornada','id_temporada','id_competicion'],axis=1)
    partidos = pd.concat([df_partidos,partidos_nuevo],ignore_index=True).drop_duplicates()
    ...
    df_nombre_no_ambiguo = df_nombres[df_nombres['nombre_ambiguo']==False].loc[:,['id_jugador','nombre_jugador','id_temporada']]
    df_nombre_ambiguo = df_nombres[df_nombres['nombre_ambiguo']].loc[:,['id_jugador','nombre_jugador','id_equipo','id_temporada']]
    ...
    tiros_nuevo['id_jugador'] = tiros_nuevo.apply(corregir_nombre_ambiguo, axis=1, args=(estructura,))
    ...
    tiros_nuevo = tiros_nuevo[['id_jugador','x','y','descripcion','anotado','cuarto','tiempo','id_partido','puntos_local','puntos_visitante']]
    tiros = actualizar_dataframe(df_tiros,tiros_nuevo,'id_tiro')
...
if __name__ == "__main__":
    ...
    procesar_jornada(args.jornada, args.competicion)